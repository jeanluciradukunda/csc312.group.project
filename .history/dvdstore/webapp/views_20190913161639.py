...
def checkout(request):
    ...
    users_ID=request.POST['numDaysBooked']
    MovieTitle=request.POST['numDaysBooked']
    bill=numOfDays*dvdPrice
    DVD.objects.filter(id=dvdID).update(NumDaysBooked=numOfDays,InStock=False)
    
    TransactionNumber="0989"
    #RentDate
    #DueDate
    
    #Payment_Method
    #Amount
    trans = Transaction.objects.create(users_ID=user_ID, TransactionNumber=, RentDate=, DueDate=, MovieTitle=MovieTitle, Payment_Method=payment,Amount=bill)
    trans.save()
