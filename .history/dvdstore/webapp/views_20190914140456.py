    ...
    if request.method == 'POST':
        ...
        if password1 == password2 :
            if Customer.objects.filter(username=username).exists():
                ...
                messages.info(request, 'Email Taken')
                return redirect('register.html')
            user = Customer.objects.create_user(phone_number=phone_number, address=,identification=,username=username, password=password1, email=email, first_name=first_name, last_name=last_name)
