...
def updateCustomer(request):
    if request.method == 'POST':
        ...
        user = Customer.objects.filter(id=dvuserID).update(phone_number=phone_number, address=address,identification=identification,username=username, email=email, first_name=first_name, last_name=last_name)
        # customer = Customer.objects.create_user(phone_number=phone_number,identification=identification,address=address)
        user.save()
        return redirect('home')
