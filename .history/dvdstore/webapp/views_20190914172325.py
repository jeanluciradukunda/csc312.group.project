...
from .form import DocumentForm, CustomerForm
import datetime
...
def clerk(request):
    ...
    context_dict = { 'dvds':dvds ,'form': form, 'trans':trans, 'users':users, 'customer':customer,'addCustomerForm':addCustomerForm}
...
def register2(request):
    ...
    if request.method == 'POST':
        ...
        user = User.objects.create_user(username=username, password=password1, email=email, first_name=first_name, last_name=last_name,is_staff)
        user.save()
...
def user_detail(request):
    ...
    if detail2.exists():
        ...
        return render(request, 'user_detail.html',{'detail1':detail1 ,'address':address, 'phone_number':phone_number,'identification':identification})
